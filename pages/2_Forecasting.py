...
with snap1:
    val_text = f"${pred_val:.2f}" if pred_val is not None else "N/A"
    st.metric(
        label="Predicted ICP",
        value=val_text,
        delta=f"{delta_pct:+.2f}%" if pred_val is not None else None,
    )
...
with snap4:
    st.metric(
        ...
        value="95%" if pred_val is not None else "N/A",
    )
...
margin-bottom: 1rem;
">
<div style="font-size: 0.85rem; color: #1e40af; margin-bottom: 0.5rem;">Forecast Range</div>
<div style="font-size: 1.5rem; font-weight: 700; color: #002b5c;">{confidence_range}</div>
<div style="font-size: 0.82rem; margin-top: 0.6rem; color: #475569;">Rentang estimasi berdasarkan error historis model</div>
</div>
""",
...
">
<div style="font-size: 0.85rem; color: #166534; margin-bottom: 0.5rem;">Primary Market Driver</div>
<div style="font-size: 1.4rem; font-weight: 700; color: #166534;">WTI Price</div>
<div style="font-size: 0.82rem; margin-top: 0.6rem; color: #475569;">Harga minyak global masih menjadi variabel paling dominan terhadap prediksi ICP.</div>
</div>
""",
        unsafe_allow_html=True,
    )
...
st.caption("Visualisasi harga ICP historis, titik prediksi model, dan area ketidakpastian forecast.")
